    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6 md:p-10 space-y-10 max-w-[1400px] mx-auto">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-sidebar-border/30 pb-10">
                <div>
                    <p class="text-[10px] font-black uppercase tracking-[0.4em] text-safari-gold mb-3">Welcome Page</p>
                    <h1 class="text-4xl font-black uppercase tracking-tighter">Hero <span class="text-safari-gold/30">Slides</span></h1>
                    <p class="text-xs opacity-40 mt-2 uppercase tracking-widest">Manage the rotating background images on the home page hero section</p>
                </div>
                <button
                    @click="showAddForm = !showAddForm"
                    class="flex items-center gap-3 px-6 py-3 bg-safari-gold text-black text-[9px] font-black uppercase tracking-widest hover:bg-white transition-all rounded-sm"
                >
                    <Plus class="w-4 h-4" />
                    Add Slide
                </button>
            ...
            <Transition enter-active-class="transition-all duration-300" enter-from-class="opacity-0 -translate-y-4" leave-active-class="transition-all duration-200" leave-to-class="opacity-0 -translate-y-4">
                <form v-if="showAddForm" @submit.prevent="addSlide" class="bg-safari-gold/5 border border-safari-gold/20 rounded-sm p-8 space-y-6">
                    <p class="text-[10px] font-black uppercase tracking-[0.4em] text-safari-gold">New Slide</p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-[10px] font-black uppercase tracking-widest opacity-60">Slide Title</label>
                            ...
                            <p v-if="addForm.errors.title" class="text-red-400 text-[10px]">{{ addForm.errors.title }}</p>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black uppercase tracking-widest opacity-60">Image URL</label>
                            ...
                            <p v-if="addForm.errors.image_url" class="text-red-400 text-[10px]">{{ addForm.errors.image_url }}</p>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black uppercase tracking-widest opacity-60">Sort Order</label>
                        ...
                        <div class="flex items-center gap-4 pt-6">
                            <label class="flex items-center gap-3 cursor-pointer">
                                ...
                                <span class="text-[10px] font-black uppercase tracking-widest opacity-60">Active</span>
                            </label>
                        </div>
                    </div>
                    <!-- Preview -->
                    <div v-if="addForm.image_url" class="aspect-[21/7] rounded-sm overflow-hidden border border-white/10 relative">
                        ...
                        <div class="absolute inset-0 flex items-center justify-center">
                            <span class="text-[10px] font-black uppercase tracking-widest opacity-60">Preview</span>
                        </div>
                    </div>
                    <div class="flex gap-4">
                        <button type="submit" :disabled="addForm.processing" class="px-8 py-3 bg-safari-gold text-black text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all rounded-sm disabled:opacity-50">
                            {{ addForm.processing ? 'Saving...' : 'Save Slide' }}
                        </button>
                        <button type="button" @click="showAddForm = false" class="px-8 py-3 border border-white/10 text-[10px] font-black uppercase tracking-widest hover:border-white/30 transition-all rounded-sm">
                            Cancel
                        </button>
                    </div>
                </form>
                ...
                >
                    ...
                    <template v-if="editingId !== slide.id">
                        <div class="flex items-center gap-6 p-4">
                            ...
                            <div class="flex-1 min-w-0 space-y-1">
                                ...
                                <a :href="slide.image_url" target="_blank" class="flex items-center gap-1 text-[10px] opacity-30 hover:opacity-60 hover:text-safari-gold transition-all truncate group/link max-w-sm">
                                    <ExternalLink class="w-3 h-3 flex-shrink-0" />
                                    <span class="truncate">{{ slide.image_url }}</span>
                                </a>
                                <div class="flex items-center gap-4">
                                    <span class="text-[9px] font-black uppercase tracking-widest opacity-30">Order: {{ slide.sort_order }}</span>
                                    <span :class="['text-[9px] font-black uppercase tracking-widest', slide.is_active ? 'text-green-500' : 'text-red-400/60']">
                            ...
                            <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button @click="toggleActive(slide)" :title="slide.is_active ? 'Deactivate' : 'Activate'" class="p-2 border border-white/10 hover:border-safari-gold/40 rounded-sm transition-all">
                                    ...
                                    <EyeOff v-else class="w-4 h-4 opacity-40" />
                                </button>
                                <button @click="startEdit(slide)" class="px-4 py-2 border border-white/10 text-[9px] font-black uppercase tracking-widest hover:border-safari-gold/40 hover:text-safari-gold transition-all rounded-sm">
                                    Edit
                                </button>
                    ...
                    <template v-else>
                        <form @submit.prevent="saveEdit(slide.id)" class="p-6 space-y-4 bg-safari-gold/5 border-t border-safari-gold/20">
                            <p class="text-[10px] font-black uppercase tracking-[0.4em] text-safari-gold">Editing Slide</p>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-1">
                                    <label class="text-[10px] font-black uppercase tracking-widest opacity-60">Title</label>
                            ...
                            <div class="flex gap-3">
                                <button type="submit" :disabled="editForm.processing" class="px-6 py-2 bg-safari-gold text-black text-[9px] font-black uppercase tracking-widest hover:bg-white transition-all rounded-sm disabled:opacity-50">
                                    {{ editForm.processing ? 'Saving...' : 'Save Changes' }}
                                </button>
                                <button type="button" @click="cancelEdit" class="px-6 py-2 border border-white/10 text-[9px] font-black uppercase tracking-widest hover:border-white/30 transition-all rounded-sm">
                                    Cancel
                                </button>
                            </div>
                        </form>
            ...
            <div v-else class="py-32 text-center border border-dashed border-white/5 rounded-sm">
                ...
                <p class="text-[11px] font-black uppercase tracking-[0.5em] opacity-20">No hero slides configured</p>
                <p class="text-[10px] opacity-20 mt-2">Add your first slide above to get started</p>
            </div>
