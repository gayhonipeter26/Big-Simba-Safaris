...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Update Operation', href: `/admin/strategic-events/${props.strategicEvent.id}/edit` },
];

const form = useForm({
    name: props.strategicEvent.name,
    occasion: props.strategicEvent.occasion || '',
    month: props.strategicEvent.month,
    days: props.strategicEvent.days.join(', '),
    color: props.strategicEvent.color,
    label: props.strategicEvent.label,
    location: props.strategicEvent.location,
    region: props.strategicEvent.region || '',
    time: props.strategicEvent.time || '',
    description: props.strategicEvent.description || '',
    asset_class: props.strategicEvent.asset_class || '',
    image: props.strategicEvent.image || '',
    status: props.strategicEvent.status,
});
...
const saveEvent = () => {
    ...
    form.put(route('admin.strategic-events.update', { strategic_event: props.strategicEvent.id }));
};
</script>

<template>
    <Head title="Update Operation" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="px-6 py-12 md:p-12 max-w-4xl mx-auto space-y-8">
             <div class="border-b border-white/10 pb-6 flex items-center justify-between">
                  <div>
                       <h1 class="font-display text-2xl font-black uppercase tracking-widest text-safari-gold">Update Timeline</h1>
                       <p class="text-xs opacity-60 uppercase tracking-widest mt-2">Adjust tracking parameters for {{ strategicEvent.name }}</p>
                  </div>
                  <CalendarDays class="w-8 h-8 opacity-20" />
             </div>

             <form @submit.prevent="saveEvent" class="space-y-8 p-8 border border-white/5 bg-black/40 backdrop-blur-md rounded-sm shadow-2xl relative overflow-hidden">
                  
                  <!-- Background Accent -->
                  <div :class="`absolute top-0 right-0 w-64 h-64 blur-[100px] opacity-[0.03] rounded-full pointer-events-none ${form.color}`"></div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                       ...
                       <div class="space-y-6">
                            <div class="space-y-2">
                                ...
                                <input v-model="form.name" type="text" class="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm focus:border-safari-gold focus:ring-1 focus:ring-safari-gold transition-all rounded-sm placeholder-white/20 uppercase tracking-wider" required />
                                <div v-if="form.errors.name" class="text-red-500 text-xs mt-1">{{ form.errors.name }}</div>
                            </div>

                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-widest opacity-80">REAL-WORLD OCCASION</label>
                                <input v-model="form.occasion" type="text" class="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm focus:border-safari-gold focus:ring-1 focus:ring-safari-gold transition-all rounded-sm placeholder-white/20 tracking-wider" />
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="space-y-2">
                                     ...
                                     <input v-model="form.days" type="text" class="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm focus:border-safari-gold focus:ring-1 focus:ring-safari-gold transition-all rounded-sm placeholder-white/20 tracking-wider" required />
                                </div>
                            </div>
                       ...
                       <div class="space-y-6">
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-2">
                                     ...
                                     <input v-model="form.location" type="text" class="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm focus:border-safari-gold focus:ring-1 focus:ring-safari-gold transition-all rounded-sm placeholder-white/20 tracking-wider" required />
                                </div>
                                <div class="space-y-2">
                                     <label class="text-[10px] font-black uppercase tracking-widest opacity-80">REGION</label>
                                     <input v-model="form.region" type="text" class="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm focus:border-safari-gold focus:ring-1 focus:ring-safari-gold transition-all rounded-sm placeholder-white/20 tracking-wider" />
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-2">
                                     ...
                                     <input v-model="form.label" type="text" class="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm focus:border-safari-gold focus:ring-1 focus:ring-safari-gold transition-all rounded-sm placeholder-white/20 tracking-wider uppercase" required />
                                </div>
                                <div class="space-y-2">
                                     <label class="text-[10px] font-black uppercase tracking-widest opacity-80">ASSET CLASS</label>
                                     <input v-model="form.asset_class" type="text" class="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm focus:border-safari-gold focus:ring-1 focus:ring-safari-gold transition-all rounded-sm placeholder-white/20 tracking-wider uppercase" />
                                </div>
                            </div>

                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-widest opacity-80">OPERATIONAL HOURS</label>
                                <input v-model="form.time" type="text" class="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm focus:border-safari-gold focus:ring-1 focus:ring-safari-gold transition-all rounded-sm placeholder-white/20 tracking-wider" />
                            </div>
                       </div>
                  </div>
                  ...
                  <div class="space-y-6 pt-6 border-t border-white/10 relative z-10">
                       ...
                       <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                 ...
                                 <input v-model="form.image" type="url" class="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm focus:border-safari-gold focus:ring-1 focus:ring-safari-gold transition-all rounded-sm placeholder-white/20 tracking-wider" />
                            </div>
                  ...
                  <div class="flex items-center justify-end gap-4 pt-8 border-t border-white/10 relative z-10">
                       ...
                       <button type="submit" :disabled="form.processing" class="px-8 py-3 bg-safari-gold text-black text-xs font-black uppercase tracking-widest hover:bg-white transition-all shadow-[0_0_15px_rgba(212,175,55,0.3)] disabled:opacity-50 flex items-center gap-3">
                            <Save class="w-4 h-4" /> {{ form.processing ? 'ENCRYPTING...' : 'UPDATE SYSTEM' }}
                       </button>
                  </div>
             </form>
        </div>
