    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6 md:p-10 space-y-10 max-w-[1600px] mx-auto">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-10">
                 <div>
                      <h1 class="text-3xl font-black uppercase tracking-tighter mb-2">Safari Inventory</h1>
                      <p class="text-xs opacity-50 uppercase tracking-widest">Manage your Kenyan expedition packages</p>
                 </div>
                 <Link :href="route('admin.tours.create')" class="px-8 py-3 bg-safari-gold text-black text-[10px] font-black uppercase tracking-widest hover:bg-black hover:text-white transition-all rounded-sm flex items-center gap-2">
                 ...
                 <div v-for="tour in tours" :key="tour.id" class="group bg-card border border-white/5 rounded-sm overflow-hidden hover:border-safari-gold/30 transition-all duration-700 flex flex-col">
                      <div class="aspect-video relative overflow-hidden">
                           ...
                           <div class="absolute top-4 left-4">
                                <span class="px-3 py-1 bg-black/80 backdrop-blur-md text-safari-gold text-[8px] font-black uppercase tracking-widest rounded-full border border-safari-gold/20">
                      ...
                      <div class="p-8 flex-1 flex flex-col justify-between">
                           <div>
                                <h3 class="text-xl font-black uppercase tracking-tight mb-6 group-hover:text-safari-gold transition-colors">{{ tour.name }}</h3>
                                <div class="grid grid-cols-2 gap-4 mb-8">
                                     ...
                                     <div class="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-safari-gold">
                           ...
                           <div class="flex items-center justify-between pt-6 border-t border-white/5">
                                <div class="flex items-center gap-2">
                                     <Link :href="route('admin.tours.edit', { tour: tour.id })" class="p-2 border border-white/5 hover:border-safari-gold/50 hover:bg-safari-gold/5 transition-all group/btn">
                                          <Edit2 class="w-3.5 h-3.5 opacity-40 group-hover/btn:opacity-100 group-hover/btn:text-safari-gold" />
                                     </Link>
                                     <button @click="deleteTour(tour.id)" class="p-2 border border-white/5 hover:border-red-500/50 hover:bg-red-500/5 transition-all group/btn">
                                          <Trash2 class="w-3.5 h-3.5 opacity-40 group-hover/btn:opacity-100 group-hover/btn:text-red-500" />
                                     </button>
                                </div>
                                <Link :href="route('tours.show', tour.slug)" target="_blank" class="text-[9px] font-black uppercase tracking-widest opacity-40 hover:opacity-100 flex items-center gap-2">
            ...
            <div v-else class="py-32 border border-dashed border-white/10 rounded-sm flex flex-col items-center justify-center text-center">
                 ...
                 <h3 class="text-xl font-black uppercase tracking-widest mb-2">No Expeditions Found</h3>
                 <p class="text-xs opacity-40 max-w-xs mb-8 uppercase tracking-widest">Your wilderness catalog is currently empty.</p>
                 <Link :href="route('admin.tours.create')" class="px-10 py-4 border border-white/20 text-[10px] font-black uppercase tracking-[0.3em] hover:bg-white hover:text-black transition-all">
                      Add Your First Tour
                 </Link>
            </div>
        </div>
