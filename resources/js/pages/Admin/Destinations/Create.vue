...
import { Plus, Save, ArrowLeft, Camera, Trash2, Globe } from 'lucide-vue-next';
import { watch } from 'vue';

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Admin', href: '/admin' },
    { title: 'Destinations', href: '/admin/destinations' },
    { title: 'New Region', href: '/admin/destinations/create' },
];
        ...
        <div class="p-6 md:p-10 space-y-12 max-w-3xl mx-auto pb-44">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-10">
                 <div class="flex items-center gap-6">
                      ...
                      <div>
                           <h1 class="font-display text-3xl font-black uppercase tracking-tighter mb-2">Build Region</h1>
                           <p class="text-xs opacity-90 uppercase tracking-widest">Mark a new geographic legend</p>
                      </div>
                 </div>
                 <button @click="submit" :disabled="form.processing" class="px-10 py-4 bg-safari-gold text-black text-sm font-black uppercase tracking-widest hover:bg-black hover:text-white transition-all rounded-sm flex items-center gap-4 group">
                      <Save class="w-4 h-4" />
                      {{ form.processing ? 'Scribing...' : 'Establish Region' }}
                 </button>
            </div>
            ...
            <form @submit.prevent="submit" class="bg-card border border-white/5 p-10 rounded-sm space-y-10">
                 <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div class="space-y-2">
                           <label class="text-sm font-bold uppercase tracking-widest opacity-80">Region Name</label>
                      ...
                      <div class="space-y-2">
                           <label class="text-sm font-bold uppercase tracking-widest opacity-80">Map Slug</label>
                           <input v-model="form.slug" type="text" class="bg-white/5 border border-white/10 rounded-sm px-6 py-4 outline-none focus:border-safari-gold transition-colors text-xs opacity-90 w-full" placeholder="samburu-reserve" />
                           <p v-if="form.errors.slug" class="text-xs text-red-500">{{ form.errors.slug }}</p>
                      </div>
                 </div>

                 <div class="space-y-2">
                      <label class="text-sm font-bold uppercase tracking-widest opacity-80">Region Lore (Description)</label>
                      <textarea v-model="form.overview" rows="5" class="bg-white/5 border border-white/10 rounded-sm px-6 py-4 outline-none focus:border-safari-gold transition-colors text-xs opacity-90 w-full" placeholder="The red sands of Samburu hold secrets..."></textarea>
                      <p v-if="form.errors.overview" class="text-xs text-red-500">{{ form.errors.overview }}</p>
                 </div>

                 <div class="space-y-2">
                      <label class="text-sm font-bold uppercase tracking-widest opacity-80">Cover Image URL</label>
                      <div class="flex items-center gap-4">
                           ...
                           <input v-model="form.image" type="text" class="bg-white/5 border border-white/10 rounded-sm px-6 py-4 outline-none focus:border-safari-gold transition-colors text-sm flex-1" placeholder="Unsplash Photo URL" />
                      </div>
