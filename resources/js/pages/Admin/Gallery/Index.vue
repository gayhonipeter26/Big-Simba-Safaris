    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="p-6 md:p-10 space-y-10 max-w-[1600px] mx-auto">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-10">
                 <div>
                      <h1 class="text-3xl font-black uppercase tracking-tighter mb-2 text-safari-gold">Wildlife Gallery Register</h1>
                      <p class="text-xs opacity-50 uppercase tracking-widest italic">Curate the visual narrative of the wild</p>
                 </div>
                 ...
                 <div v-for="item in items" :key="item.id" class="group relative aspect-video bg-card border border-white/5 rounded-sm overflow-hidden hover:border-safari-gold/30 transition-all duration-700 flex flex-col shadow-2xl">
                      <div class="absolute inset-0">
                           ...
                           <div v-else class="absolute inset-0 bg-black/80 flex items-center justify-center">
                                <Video class="w-16 h-16 text-safari-gold opacity-40 group-hover:opacity-100 transition-opacity" />
                           </div>
                      ...
                      <div class="absolute bottom-0 left-0 right-0 p-10 z-20 space-y-6 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                           <div>
                                <div class="flex items-center gap-3 text-[9px] font-black uppercase tracking-[0.4em] text-safari-gold mb-3">
                                     <ImageIcon v-if="item.type === 'image'" class="w-3.5 h-3.5" />
                                     <Video v-else class="w-3.5 h-3.5" />
                                     {{ item.type }}
                                </div>
                                <h3 class="text-2xl font-black uppercase tracking-tight leading-tight mb-2 group-hover:text-safari-gold transition-colors">{{ item.title }}</h3>
                                <p class="text-xs opacity-50 line-clamp-1 italic max-w-md">{{ item.description }}</p>
                           </div>
                           ...
                           <div class="flex items-center gap-4 pt-6 border-t border-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700">
                                <button @click="openEditModal(item)" class="flex-1 py-3 bg-white text-black text-[9px] font-black uppercase tracking-[0.3em] hover:bg-safari-gold transition-all flex items-center justify-center gap-2">
                                     <Edit2 class="w-3 h-3" /> Edit
                                </button>
                                <button @click="deleteItem(item.id)" class="px-6 py-3 border border-red-500/30 text-red-500 text-[9px] font-black uppercase tracking-[0.3em] hover:bg-red-500 hover:text-white transition-all">
                                     <Trash2 class="w-3.5 h-3.5" />
                                </button>
                                <a :href="item.url" target="_blank" class="px-6 py-3 bg-white/5 border border-white/10 text-white text-[9px] font-black uppercase tracking-[0.3em] hover:bg-white/10 transition-all flex items-center gap-2">
            ...
            <div v-else class="py-44 border border-dashed border-white/10 rounded-sm flex flex-col items-center justify-center text-center">
                 ...
                 <h3 class="text-2xl font-black uppercase tracking-widest mb-4">Gallery is Vacant</h3>
                 <p class="text-xs opacity-40 max-w-sm mb-12 uppercase tracking-widest leading-relaxed italic">The pride's visual register is currently empty. Start by adding images or video links.</p>
        ...
        <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl animate-in fade-in duration-500">
             <div class="w-full max-w-2xl bg-card border border-white/5 rounded-sm p-12 space-y-12 shadow-[0_0_100px_rgba(0,0,0,0.8)] relative">
                  ...
                  <div class="text-center space-y-2 pb-8 border-b border-white/5">
                       <h2 class="text-2xl font-black uppercase tracking-tighter text-safari-gold">
                            {{ isEditing ? 'Refine Gallery Asset' : 'New Asset Registration' }}
                       </h2>
                       <p class="text-[10px] opacity-40 uppercase tracking-[0.4em] italic">- Curating the Visual Experience //</p>
                  </div>

                  <form @submit.prevent="submit" class="space-y-10">
                       <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div class="space-y-4">
                                 <label class="text-[10px] font-black uppercase tracking-[0.3em] opacity-40">Asset Title</label>
                            ...
                            <div class="space-y-4">
                                 <label class="text-[10px] font-black uppercase tracking-[0.3em] opacity-40">Asset Type</label>
                       ...
                       <div class="space-y-4">
                            <label class="text-[10px] font-black uppercase tracking-[0.3em] opacity-40">Resource URL (Direct Link)</label>
                       ...
                       <div class="space-y-4">
                            <label class="text-[10px] font-black uppercase tracking-[0.3em] opacity-40">Description</label>
                       ...
                       <div class="flex items-center gap-4">
                            ...
                            <label for="featured" class="text-[10px] font-black uppercase tracking-[0.3em] cursor-pointer">Feature on Homepage</label>
                       </div>

                       <div class="pt-8 flex flex-col items-center gap-6">
                            <button type="submit" :disabled="form.processing" class="w-full py-6 bg-safari-gold text-black text-[12px] font-black uppercase tracking-[0.5em] hover:bg-white transition-all shadow-[0_0_50px_rgba(202,138,4,0.2)]">
                                 {{ isEditing ? 'Sync Changes' : 'Finalize Registration' }}
                            </button>
                            <button @click="showModal = false" type="button" class="text-[9px] font-black uppercase tracking-[0.4em] opacity-20 hover:opacity-100 transition-opacity">Abort Mission</button>
                       </div>
                  </form>
             </div>
        </div>
